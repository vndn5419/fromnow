  ...
  this.gapi.client.request({
    ...
    callback: function(response) {
      if (response.error) {
        ...
        $('#signoutButton').show(); //로그아웃 버튼 
        $('#signinButton').hide(); //로그인 버튼
      }
    }.bind(this)
  });
  ...
  var uploader = new MediaUploader({
    ...
    onProgress: function(data) {
      ...
      var percentageComplete = (bytesUploaded * 100) / totalBytes;
		...
		$('#progressbar').text(Math.floor(percentageComplete)+"%");	
...
UploadVideo.prototype.handleUploadClicked = function() {
	...
	if($('#title').val() === '') {
		alert('Please type title of your video');
		return;
	}
      ...
      if (response.error) {
        ...
        switch (uploadStatus) {
          ...
          case 'processed':
            ...
            //$('#bm').css('height', '0px');
            $('#inputarea').html('<p class="youtube"><iframe width="448" height="252" src="https://www.youtube.com/embed/' + this.videoId + '" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe><br></p><button class="deleteVideo" onclick="deleteVideo()">Delete</button><br>'); //게시글에 등록            
...
function deleteVideo(){
	...
	//alert('비디오id : '+videoId+', 토큰 : '+accesstoken);
	if(videoId.length == 0){
		//alert('삭제할 영상이 없습니다.');
		return;
	}
    $.ajax({
        url: "https://www.googleapis.com/youtube/v3/videos?part=id&id="+videoId+"&access_token="+accesstoken,
        type: 'DELETE',
